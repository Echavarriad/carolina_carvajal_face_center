                    ...
                    <div class="nav-tabs-custom">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade in active" id="tab_1">
                                    ...
                                    <div class="col-lg-6">
                                        ...
                                        <select name="code_state" id="state" class="form-control">
                                            ...
                                            @foreach ($states as $item)
                                                <option value="{{ $item->id_state }}" data-cities="{{ json_encode($item->cities) }}" {{ $record->code_state == $item->id_state ? ' selected="selected"' : '' }}>{{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                        <br>
                                    </div>
                                ...
                                <input type="text" name="value_min_buy" class="form-control" value="{{ $record->value_min_buy }}">
                                <br>

                                <label>Valor del envío</label>
                                <input type="text" name="shipping_fee" class="form-control" value="{{ $record->shipping_fee }}">
                            </div>
...
<script>
    ...
    let code_city= '{{ $record->code_city }}';
    let options= '';
    state.cities.forEach((city, index) => {
        let selected= code_city == city.ciudad ? 'selected' : '';
        options += `<option value="${ city.ciudad }" ${selected}>${ city.name }</option>`;
    });
