                ...
                <tr>
                  ...
                  <td class="btn-actions-index">
                    @if($user->id != auth('admin')->user()->id)
                      <input type="checkbox" name="{{ $user->id }}" class="input-switch" {{ $user->published ? 'checked':''}} data-size="mini" data-url="{{ route('admins.published') }}">
                    @endif
                  </td>
                  <td><a href="{{ route('admins.edit' , [$user->id]) }}" class="btn btn-primary btn-flat" title="Editar"><i class="fa fa-edit"></i></a>
                   @if($user->id != auth('admin')->user()->id)
